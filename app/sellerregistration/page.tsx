...
const RegisterPage = () => {
  const [selectedRole, setSelectedRole] = useState<'SELLER' | 'USER'>('SELLER');
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    phoneNumber: '',
    company: '',  // Added for seller registration
  });
  ...
  const handleNextStep = () => {
    if (formStep === 0) {
      ...
      if (!formData.email || !formData.phoneNumber || !formData.company && selectedRole) {
        setErrors(prev => ({
          ...prev,
          email: !formData.email ? 'Email is required' : '',
          phoneNumber: !formData.phoneNumber ? 'Phone Number is required' : '',
          company: selectedRole === 'SELLER' && !formData.company ? 'Company is required' : '',
        }));
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.username || !formData.password || !formData.confirmPassword || !formData.email || !formData.phoneNumber || !formData.company) {
    ...
    if (response.ok) {
      alert(data.message);
      window.location.href = '/sellerlogin';  // Redirect after successful registration
    } else {
      ...
      <div className="w-full max-w-md md:max-w-4xl bg-white shadow-lg rounded-lg overflow-hidden flex flex-col md:flex-row-reverse h-auto md:h-[75vh]">
        ...
        <div className="md:w-1/2 w-full p-4 md:p-10 flex flex-col justify-center items-center">
          <div className="w-full max-w-md">
            ...
            <p className="text-lg md:text-xl text-center text-red-800 mb-4"> Seller Registration</p>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {formStep === 1 && (
                <>
                  ...
                  {selectedRole === 'SELLER' && (
                    <div className="mb-3 sm:mb-4">
                      <label className="block font-semibold mb-2 text-herit">Company</label>
                      <input
                        type="text"
                        name="company"
                  ...
                  <div className="flex justify-end">
                    ...
                    <button
                      type="submit"
                      className="py-2 px-4 rounded-md bg-herit text-white hover:bg-opacity-80 transition-colors duration-300"
                    >
                      Register
                    </button>
                  </div>
